...
int290 <- read.csv("Stats-Final-Project/int290.csv")  %>%  
  mutate(PPID = as.character(PPID))
int290long <- read.csv("Stats-Final-Project/int290long.csv")
# the 226 students who completed 
complet226 <- read.csv("Stats-Final-Project/complet226.csv")%>%  
  mutate(PPID = as.character(PPID))
complet226long <- read.csv("Stats-Final-Project/complet226long.csv")
```
...
```
Of the 328 students who consented to participate, 226 (70%; 118 Control and 108 Affirmed) students completed the affirmation exercise as well as both the pre and post-semester questionnaires
...
```{r}
fclose1.coef.obs <- modfclose1$coefficients[2] # coefficient of effect of intervention condition on closeness centrality
fclose1.coef.out <- vector("numeric")

for(i in 1:numrands){   
  randnodes <- sample(rownames(fmat1), replace = FALSE)   # randomly order node labels
  rownames(fmat1r) <- randnodes                           # assign randomly ordered nodes to row names of matrix
  colnames(fmat1r) <- randnodes                           # assign randomly ordered nodes to column names of matrix
...
```{r}
snat1_interest<- subset(SNA_T1, select= c(PPID, Friend1Strength, Friend2Strength, Friend3Strength, Friend4Strength, Friend5Strength, Friend6Strength)) #selecting friendship strengths
snat1_interest[is.na(snat1_interest)] <- 0
snat1_interest$Friendship <- rowSums(snat1_interest[ , c(2:7)]) #summing up friendship strengths
t1 <- subset(snat1_interest, select= c(PPID, Friendship))%>%  #selecting participant ID and friendship 
  mutate(PPID = as.character(PPID))

snat2_interest<- subset(SNA_T2, select= c(PPID, Friend1Strength, Friend2Strength, Friend3Strength, Friend4Strength, Friend5Strength, Friend6Strength))
...
summary(glm(Y ~ X, family = "binomial")) #does x cause y in the overall sense? yes
summary(glm(Y ~ X + Z, family = "binomial")) #but when u add in the mediator z, now it looks like x does not cause y. power of x disappears. 
